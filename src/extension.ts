...
// ─── Types ──────────────────────────────
interface Goal { target: number; unit: string; label: string; }
interface ProjectStats { name: string; path: string; totalMs: number; totalLines: number; totalFiles: number; totalSaves: number; sessions: number; languages: Record<string, number>; lastOpened: number; firstSeen: number; daysActive: Set<string>; }
interface Goals { daily: { time: Goal; lines: Goal; sessions: Goal }; weekly: { time: Goal; days: Goal }; monthly: { time: Goal; days: Goal }; }
interface GitStats { available: boolean; branch: string; commitsToday: number; commitsWeek: number; commitsMonth: number; commitsTotal: number; filesChanged: number; recentMessages: string[]; repoName: string; }

// ─── Constants ──────────────────────────
const IDLE_TIMEOUT_MS = 120_000;
const FOCUS_SESSION_MIN_MS = 25 * 60_000;
const ACTIVE_TICK_MS = 5_000;
const SAVE_INTERVAL_MS = 30_000;
const STATUS_UPDATE_MS = 10_000;
const GIT_CACHE_MS = 60_000;

// ─── State ──────────────────────────────
let storagePath = '';
let sessionMs = 0; let sessionLines = 0; let sessionFiles = 0; let sessionSaves = 0;
let sessionLanguages: Record<string, number> = {}; let focusSessions = 0; let focusStart = 0;
let savedData: Record<string, unknown> = {}; let projects: Record<string, ProjectStats> = {};
let activeProject: string | null = null; let lastActivity = Date.now();
let statusBar: vscode.StatusBarItem; let panel: vscode.WebviewPanel | null = null;
let cachedGitStats: GitStats | null = null; let lastGitCheck = 0;
...
function log(msg: string): void { outputChannel.appendLine(`[${new Date().toISOString()}] ${msg}`); }

// ─── Utility functions ──────────────────
const todayStr = (): string => new Date().toISOString().slice(0, 10);
const weekStartStr = (o = 0): string => { const d = new Date(); d.setDate(d.getDate() - d.getDay() - o * 7); return d.toISOString().slice(0, 10); };
const monthStartStr = (): string => new Date().toISOString().slice(0, 7);
function formatDuration(ms: number): string { const m = Math.floor(ms / 60_000); return m < 60 ? `${m}m` : `${Math.floor(m / 60)}h ${m % 60}m`; }
function formatNumber(n: number): string { return n >= 1_000 ? `${(n / 1_000).toFixed(1)}K` : String(n); }
function pct(c: number, t: number): number { return Math.min(Math.round((c / t) * 100), 100); }
function getProjectName(wp: string): string { const f = vscode.workspace.workspaceFolders; if (f) for (const w of f) if (wp.startsWith(w.uri.fsPath)) return w.name; return path.basename(wp) || 'Unknown Project'; }
function getActiveWorkspace(): string | null { const e = vscode.window.activeTextEditor; if (e) { const dp = e.document.uri.fsPath; const f = vscode.workspace.workspaceFolders; if (f) for (const w of f) if (dp.startsWith(w.uri.fsPath)) return w.uri.fsPath; } const f = vscode.workspace.workspaceFolders; return f?.[0]?.uri.fsPath ?? null; }
function getGoalsConfig(): Goals { const c = vscode.workspace.getConfiguration('devpulse.goals'); return { daily: { time: { target: c.get<number>('dailyTime', 240), unit: 'min', label: 'Coding Time' }, lines: { target: c.get<number>('dailyLines', 500), unit: 'lines', label: 'Lines' }, sessions: { target: c.get<number>('dailySessions', 3), unit: 'sessions', label: 'Focus Sessions' } }, weekly: { time: { target: c.get<number>('weeklyTime', 1_200), unit: 'min', label: 'Weekly Time' }, days: { target: c.get<number>('weeklyDays', 5), unit: 'days', label: 'Days Coded' } }, monthly: { time: { target: c.get<number>('monthlyTime', 4_800), unit: 'min', label: 'Monthly Time' }, days: { target: c.get<number>('monthlyDays', 20), unit: 'days', label: 'Days Coded' } } }; }
function ensureProject(wp: string): ProjectStats { if (!projects[wp]) projects[wp] = { name: getProjectName(wp), path: wp, totalMs: 0, totalLines: 0, totalFiles: 0, totalSaves: 0, sessions: 0, languages: {}, lastOpened: Date.now(), firstSeen: Date.now(), daysActive: new Set() }; return projects[wp]; }

// ─── Data persistence ───────────────────
function loadData(): void { try { const fp = path.join(storagePath, 'devpulse.json'); if (fs.existsSync(fp)) { savedData = JSON.parse(fs.readFileSync(fp, 'utf-8')); if (savedData._projects) for (const [k, v] of Object.entries(savedData._projects as Record<string, any>)) projects[k] = { ...v, daysActive: new Set(v.daysActive || []) } as ProjectStats; } } catch { savedData = {}; } }
function saveData(): void { const k = todayStr(); if (!savedData[k]) savedData[k] = { ms: 0, lines: 0, files: 0, saves: 0, langs: {}, sessions: 0 }; const cur = savedData[k] as Record<string, any>; cur.ms += sessionMs; cur.lines += sessionLines; cur.files += sessionFiles; cur.saves += sessionSaves; cur.sessions = (cur.sessions || 0) + focusSessions; for (const [l, n] of Object.entries(sessionLanguages)) cur.langs[l] = (cur.langs[l] || 0) + n; if (activeProject) { const p = projects[activeProject]; if (p) { p.totalLines += sessionLines; p.totalFiles += sessionFiles; p.totalSaves += sessionSaves; p.sessions += focusSessions; for (const [l, n] of Object.entries(sessionLanguages)) p.languages[l] = (p.languages[l] || 0) + n; savedData._projects = savedData._projects || {}; (savedData._projects as any)[activeProject] = { name: p.name, path: p.path, totalMs: p.totalMs, totalLines: p.totalLines, totalFiles: p.totalFiles, totalSaves: p.totalSaves, sessions: p.sessions, languages: p.languages, lastOpened: p.lastOpened, firstSeen: p.firstSeen, daysActive: Array.from(p.daysActive) }; } } sessionMs = 0; sessionLines = 0; sessionFiles = 0; sessionSaves = 0; focusSessions = 0; sessionLanguages = {}; fs.writeFileSync(path.join(storagePath, 'devpulse.json'), JSON.stringify(savedData)); loadData(); }

// ─── Computed stats ─────────────────────
const todayMs = (): number => ((savedData[todayStr()] as any)?.ms || 0) + sessionMs;
const todayLines = (): number => ((savedData[todayStr()] as any)?.lines || 0) + sessionLines;
const todaySessions = (): number => ((savedData[todayStr()] as any)?.sessions || 0) + focusSessions;
const todayLangs = (): Record<string, number> => { const m: Record<string, number> = { ...((savedData[todayStr()] as any)?.langs || {}) }; for (const [l, n] of Object.entries(sessionLanguages)) m[l] = (m[l] || 0) + n; return m; };
function getStreak(): number { let s = 0; const d = new Date(); if (todayMs() === 0 && (!savedData[todayStr()] || (savedData[todayStr()] as any)?.ms === 0)) d.setDate(d.getDate() - 1); for (let i = 0; i < 400; i++) { const k = d.toISOString().slice(0, 10); if ((k === todayStr() ? todayMs() : ((savedData[k] as any)?.ms || 0)) > 0) { s++; d.setDate(d.getDate() - 1); } else break; } return s || 0; }
function bestStreak(): number { let b = 0, c = 0, p: Date | null = null; for (const k of Object.keys(savedData).filter(k => k !== '_projects').sort()) { const ms = k === todayStr() ? todayMs() : ((savedData[k] as any)?.ms || 0); if (ms > 0) { const d = new Date(k); if (p && (d.getTime() - p.getTime()) / 86_400_000 <= 1.5) c++; else c = 1; p = d; if (c > b) b = c; } else c = 0; } return b || 0; }
const totalHours = (): number => { let ms = todayMs(); for (const [k, d] of Object.entries(savedData)) { if (k !== todayStr() && k !== '_projects') ms += (d as any)?.ms || 0; } return Math.floor(ms / 3_600_000); };
const totalLines = (): number => { let n = todayLines(); for (const [k, d] of Object.entries(savedData)) { if (k !== todayStr() && k !== '_projects') n += (d as any)?.lines || 0; } return n; };
const weekMs = (): number => { const w = weekStartStr(); let ms = 0; for (const [k, d] of Object.entries(savedData)) { if (k >= w && k !== '_projects') ms += (d as any)?.ms || 0; } return ms + (todayStr() >= w ? sessionMs : 0); };
const weekDays = (): number => { const w = weekStartStr(); let d = 0; for (const [k, v] of Object.entries(savedData)) { if (k >= w && k !== '_projects' && (v as any)?.ms > 0) d++; } if (todayStr() >= w && todayMs() > 0 && !savedData[todayStr()]) d++; return d; };
const monthMs = (): number => { const m = monthStartStr(); let ms = 0; for (const [k, d] of Object.entries(savedData)) { if (k.startsWith(m) && k !== '_projects') ms += (d as any)?.ms || 0; } return ms + sessionMs; };
const monthDays = (): number => { const m = monthStartStr(); let d = 0; for (const [k, v] of Object.entries(savedData)) { if (k.startsWith(m) && k !== '_projects' && (v as any)?.ms > 0) d++; } if (todayMs() > 0 && !savedData[todayStr()]) d++; return d; };

// ─── Cross-platform Git ────────────────
function git(args: string[], cwd: string): string { try { return execFileSync('git', args, { cwd, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'], windowsHide: true }).trim(); } catch { return ''; } }
function isGitInstalled(): boolean { try { execFileSync('git', ['--version'], { stdio: 'pipe', windowsHide: true }); return true; } catch { return false; } }
function findGitRepo(): string | null { const f = vscode.workspace.workspaceFolders; if (!f?.length) return null; const r = f[0].uri.fsPath; try { execFileSync('git', ['rev-parse', '--git-dir'], { cwd: r, stdio: 'pipe', windowsHide: true }); return r; } catch { return null; } }
function countLines(text: string): number { return text ? text.split('\n').filter(l => l.length > 0).length : 0; }
function getGitStats(): GitStats {
    const now = Date.now(); if (cachedGitStats && (now - lastGitCheck) < GIT_CACHE_MS) return cachedGitStats;
    const def: GitStats = { available: false, branch: '', commitsToday: 0, commitsWeek: 0, commitsMonth: 0, commitsTotal: 0, filesChanged: 0, recentMessages: [], repoName: '' };
    if (!isGitInstalled()) return def; const rp = findGitRepo(); if (!rp) return def;
    try { const br = git(['rev-parse', '--abbrev-ref', 'HEAD'], rp); const rn = path.basename(rp); const au = git(['config', 'user.name'], rp); if (!au) { cachedGitStats = { ...def, available: true, branch: br, repoName: rn }; lastGitCheck = now; return cachedGitStats; }
        const ts = todayStr(); const ws = weekStartStr(); const ms = monthStartStr() + '-01';
        cachedGitStats = { available: true, branch: br, commitsToday: countLines(git(['log', '--oneline', `--since=${ts}T00:00:00`, `--until=${ts}T23:59:59`, `--author=${au}`], rp)), commitsWeek: countLines(git(['log', '--oneline', `--since=${ws}T00:00:00`, `--author=${au}`], rp)), commitsMonth: countLines(git(['log', '--oneline', `--since=${ms}T00:00:00`, `--author=${au}`], rp)), commitsTotal: parseInt(git(['rev-list', '--count', 'HEAD', `--author=${au}`], rp), 10) || 0, filesChanged: countLines(git(['diff', '--name-only', 'HEAD'], rp)), recentMessages: git(['log', '-5', '--pretty=format:%s', `--author=${au}`], rp).split('\n').filter(m => m.length > 0).slice(0, 5), repoName: rn }; lastGitCheck = now; return cachedGitStats; } catch { return def; }
}

// ─── Cleanup ────────────────────────────
function disposeAll(): void { if (focusTimer) clearInterval(focusTimer); if (activeTimer) clearInterval(activeTimer); if (saveTimer) clearInterval(saveTimer); if (statusTimer) clearInterval(statusTimer); for (const d of disposables) d.dispose(); if (statusBar) statusBar.dispose(); if (panel) panel.dispose(); outputChannel.dispose(); }
...
// ─── HTML Components ────────────────────
function goalBar(label: string, current: number, target: number, unit: string, done: boolean): string {
    const p = pct(current, target); const r = target - current;
    const val = unit === 'min' ? formatDuration(current * 60_000) : `${current} ${unit}`;
    const tgt = unit === 'min' ? formatDuration(target * 60_000) : `${target} ${unit}`;
    const left = unit === 'min' ? formatDuration(r * 60_000) : `${r} ${unit}`;
    const eta = current > 0 && !done ? `Est. ${formatDuration(Math.floor((r / (current / (todayMs() / 60_000))) * 60_000))}` : '';
...
function projectListHTML(sort: string): string {
    const pa = Object.values(projects); if (!pa.length) return '<div class="lempty"><strong>No projects tracked yet.</strong><br>Open a workspace folder and start coding. DevPulse will automatically detect your project.</div>';
    if (sort === 'time') pa.sort((a, b) => b.totalMs - a.totalMs); else if (sort === 'recent') pa.sort((a, b) => b.lastOpened - a.lastOpened); else pa.sort((a, b) => b.daysActive.size - a.daysActive.size);
    return pa.map(p => {
        const h = Math.floor(p.totalMs / 3_600_000); const d = p.daysActive.size;
        const fl = p.totalFiles; const topLangs = Object.keys(p.languages).slice(0, 2).join(', ');
        const ago = Math.floor((Date.now() - p.lastOpened) / 86_400_000);
...
function gitHTML(): string {
    const g = getGitStats(); if (!g.available || !g.repoName) return '<div class="lempty"><strong>No Git repository detected.</strong><br>Open a folder with a Git repository to see commits, branches, and activity.</div>';
...
function buildDashboardHTML(): string {
    const g = getGoalsConfig(); const tm = todayMs(); const tl = todayLines(); const ts = todaySessions();
    const lg = todayLangs(); const st = getStreak(); const bs = bestStreak(); const th = totalHours(); const tll = totalLines();
    const wm = weekMs(); const wd = weekDays(); const mm = monthMs(); const md = monthDays();
    const yk = new Date(Date.now() - 86_400_000).toISOString().slice(0, 10);
    const ym = (savedData[yk] as any)?.ms || 0; const tr = ym > 0 ? Math.round(((tm - ym) / ym) * 100) : 0;
    const dt = Math.floor(tm / 60_000); const dtD = dt >= g.daily.time.target;
    const dlD = tl >= g.daily.lines.target; const dsD = ts >= g.daily.sessions.target;
    const wt = Math.floor(wm / 60_000); const wtD = wt >= g.weekly.time.target; const wdD = wd >= g.weekly.days.target;
    const mt = Math.floor(mm / 60_000); const mtD = mt >= g.monthly.time.target; const mdD = md >= g.monthly.days.target;
    const git = getGitStats();

    const langs = Object.entries(lg).sort((a, b) => b[1] - a[1]); const tlT = langs.reduce((s, [, n]) => s + n, 0) || 1;
    const lc: Record<string, string> = { typescript: '#3178c6', javascript: '#f7df1e', python: '#3776ab', css: '#c6538c', html: '#e34c26', json: '#292929', markdown: '#083fa1', ts: '#3178c6', js: '#f7df1e', py: '#3776ab', java: '#b07219', go: '#00add8', rust: '#dea584', cpp: '#f34b7d' };
    let lr = ''; for (const [l, n] of langs.slice(0, 4)) { const p = Math.round((n / tlT) * 100); const c = lc[l.toLowerCase()] || '#8b949e'; lr += `<div class="lr"><span class="ld" style="background:${c}"></span><span class="ln">${l}</span><span class="lp">${p}%</span></div>`; } if (!lr) lr = '<div class="lempty"><strong>No language data yet.</strong><br>Save a file to see your language breakdown.</div>';

    let hm = ''; const nw = new Date();
    for (let w = 25; w >= 0; w--) for (let d = 6; d >= 0; d--) { const dt = new Date(nw); dt.setDate(dt.getDate() - (w * 7 + d)); const k = dt.toISOString().slice(0, 10); const it = k === todayStr(); const ms = it ? tm : ((savedData[k] as any)?.ms || 0); let lv = 0; const h = ms / 3_600_000; if (h > 0) lv = 1; if (h > 0.5) lv = 2; if (h > 2) lv = 3; if (h > 4) lv = 4; const dayData = it ? { lines: tl, files: sessionFiles, langs: lg, sessions: ts } : (savedData[k] as any); const tip = `${k}: ${formatDuration(ms)}${dayData ? ` | ${dayData.lines || 0} lines` : ''}`; hm += `<div class="hmc hl${lv}" data-tip="${tip}"${it ? ' style="outline:1px solid var(--vscode-focusBorder,#007acc)"' : ''} tabindex="0" role="img" aria-label="${tip}"></div>`; }

    const wd_: number[] = []; for (let i = 6; i >= 0; i--) { const dt = new Date(nw); dt.setDate(dt.getDate() - i); const k = dt.toISOString().slice(0, 10); wd_.push(k === todayStr() ? tm : ((savedData[k] as any)?.ms || 0)); }
    const mxW = Math.max(...wd_, 60_000); const dl_ = ['S', 'M', 'T', 'W', 'T', 'F', 'S']; let wb = '';
    for (let i = 0; i < 7; i++) { const h = wd_[i] / 3_600_000; const bp = Math.max((h / (mxW / 3_600_000)) * 48, 3); const it = i === 6; wb += `<div class="bw"><div class="b ${it ? 'bt' : h > 0 ? 'ba' : 'bi'}" style="height:${bp}px" role="img" aria-label="${dl_[(nw.getDay() - 6 + i + 7) % 7]}: ${formatDuration(wd_[i])}"></div><span class="bd${it ? ' bda' : ''}">${dl_[(nw.getDay() - 6 + i + 7) % 7]}</span></div>`; }

    // Insights
    const wk = weekStartStr(); const ek = todayStr(); let twm = 0, twl = 0, tws = 0, twls = 0; const twlg: Record<string, number> = {}; const dm: Record<string, number> = {}; const dn = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    for (const [k, d] of Object.entries(savedData)) { if (k === '_projects' || k < wk || k > ek) continue; const dy = d as any; twm += dy.ms || 0; twl += dy.lines || 0; tws += dy.sessions || 0; if (dy.ms > twls) twls = dy.ms; for (const [ln, nn] of Object.entries(dy.langs || {})) twlg[ln] = (twlg[ln] || 0) + (nn as number); const dt = new Date(k); dm[dn[dt.getDay()]] = (dm[dn[dt.getDay()]] || 0) + (dy.ms || 0); }
    twm += sessionMs; twl += sessionLines; tws += focusSessions; let bd_ = '', bm_ = 0; for (const [nn, dd] of Object.entries(dm)) { if (dd > bm_) { bm_ = dd; bd_ = nn; } }
    const lwk = weekStartStr(1); let lwm = 0, lwl = 0; for (const [k, d] of Object.entries(savedData)) { if (k === '_projects' || k < lwk || k >= wk) continue; const dy = d as any; lwm += dy.ms || 0; lwl += dy.lines || 0; }
    const tch = lwm > 0 ? Math.round(((twm - lwm) / lwm) * 100) : 0; const da = Object.keys(dm).length;
    const insightsHTML = [
...
<div class="metric"><div class="metric-value" id="dv-deep">${formatDuration(tm > 25 * 60_000 ? Math.floor(tm * 0.4) : 0)}</div><div class="metric-label">Deep Work</div></div>
</div>
...
function buildShareCardHTML(template: string): string {
    const th = totalHours(); const tl = totalLines(); const bs = bestStreak(); const cs = getStreak();
    const topLang = Object.entries(todayLangs()).sort((a, b) => b[1] - a[1])[0];
    const pc = Object.keys(projects).length; const git = getGitStats();
...
// ─── Wrapped ────────────────────────────
function buildWrappedHTML(period: 'monthly' | 'yearly'): string {
    const th = totalHours(); const tl = totalLines(); const bs = bestStreak(); const topLang = Object.entries(todayLangs()).sort((a, b) => b[1] - a[1])[0];
    const tp = Object.values(projects).sort((a, b) => b.totalMs - a.totalMs)[0]; const git = getGitStats();
...
<div class="slide sd" id="s4"><div class="anim"><div class="wl">Main project</div></div><div class="anim ad1"><div class="wv" style="font-size:44px;color:#3fb950">${tp ? tp.name : '—'}</div></div><div class="anim ad2"><div class="wl2">${tp ? Math.floor(tp.totalMs / 3_600_000) : 0}h spent</div></div></div>
...
export function activate(context: vscode.ExtensionContext): void {
    storagePath = context.globalStorageUri.fsPath; if (!fs.existsSync(storagePath)) fs.mkdirSync(storagePath, { recursive: true });
    loadData(); activeProject = getActiveWorkspace(); if (activeProject) ensureProject(activeProject);
    disposables.push(vscode.window.onDidChangeActiveTextEditor(() => { const np = getActiveWorkspace(); if (np && np !== activeProject) { activeProject = np; ensureProject(activeProject); cachedGitStats = null; } }));
    disposables.push(vscode.workspace.onDidChangeTextDocument(e => { for (const c of e.contentChanges) sessionLines += Math.max(0, c.text.split('\n').length - 1); lastActivity = Date.now(); if (!focusTimer && Date.now() - lastActivity < IDLE_TIMEOUT_MS) { focusStart = Date.now(); focusTimer = setInterval(() => { if (Date.now() - focusStart >= FOCUS_SESSION_MIN_MS) { focusSessions++; focusStart = Date.now(); } }, 60_000); } }), vscode.workspace.onDidSaveTextDocument(doc => { sessionFiles++; sessionSaves++; sessionLanguages[doc.languageId] = (sessionLanguages[doc.languageId] || 0) + 1; cachedGitStats = null; }), vscode.window.onDidChangeWindowState(s => { if (s.focused) lastActivity = Date.now(); }));
    activeTimer = setInterval(() => { if (Date.now() - lastActivity < IDLE_TIMEOUT_MS) { sessionMs += ACTIVE_TICK_MS; if (activeProject) { const p = projects[activeProject]; if (p) { p.totalMs += ACTIVE_TICK_MS; p.daysActive.add(todayStr()); } } } if (Date.now() - lastActivity >= IDLE_TIMEOUT_MS && focusTimer) { clearInterval(focusTimer); focusTimer = null; } if (panel) panel.webview.postMessage({ t: 'u', time: formatDuration(todayMs()), streak: String(getStreak()), lines: formatNumber(todayLines()), deep: formatDuration(todayMs() > 25 * 60_000 ? Math.floor(todayMs() * 0.4) : 0) }); }, ACTIVE_TICK_MS);
    saveTimer = setInterval(() => saveData(), SAVE_INTERVAL_MS);
    statusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100); statusBar.command = 'devpulse.showDashboard'; statusBar.text = '$(pulse) DevPulse'; statusBar.show(); disposables.push(statusBar);
    statusTimer = setInterval(() => { const m = Math.floor(todayMs() / 60_000); statusBar.text = m > 0 ? `$(flame) ${m}m` : '$(pulse) DevPulse'; }, STATUS_UPDATE_MS);
    disposables.push(vscode.commands.registerCommand('devpulse.showDashboard', () => { if (panel) { panel.reveal(); } else { panel = vscode.window.createWebviewPanel('devpulse', 'DevPulse', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true }); panel.onDidDispose(() => { panel = null; }); panel.webview.onDidReceiveMessage(m => { if (m.t === 'shareCard') { vscode.env.clipboard.writeText(buildShareCardHTML(m.template as string)); vscode.window.showInformationMessage('Share card copied to clipboard'); } if (m.t === 'poll' && panel) { /* Smooth update via postMessage, no full reload */ } if (m.t === 'sortProjects' && panel) panel.webview.postMessage({ t: 'projects', html: projectListHTML(m.sort as string) }); if (m.t === 'openWrapped' && panel) panel.webview.html = buildWrappedHTML((m.period as 'monthly' | 'yearly') ?? 'yearly'); }); } panel.webview.html = buildDashboardHTML(); }), vscode.commands.registerCommand('devpulse.wrapped', () => { if (panel) { panel.reveal(); } else { panel = vscode.window.createWebviewPanel('devpulse', 'DevPulse Wrapped', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true }); panel.onDidDispose(() => { panel = null; }); } panel.webview.html = buildWrappedHTML('yearly'); }), vscode.commands.registerCommand('devpulse.wrappedMonthly', () => { if (panel) { panel.reveal(); } else { panel = vscode.window.createWebviewPanel('devpulse', 'DevPulse Wrapped', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true }); panel.onDidDispose(() => { panel = null; }); } panel.webview.html = buildWrappedHTML('monthly'); }), vscode.commands.registerCommand('devpulse.shareCard', () => { vscode.env.clipboard.writeText(buildShareCardHTML('professional')); vscode.window.showInformationMessage('Share card copied to clipboard'); }), vscode.commands.registerCommand('devpulse.exportData', () => { saveData(); const fp = path.join(storagePath, `devpulse-export-${todayStr()}.json`); fs.writeFileSync(fp, JSON.stringify(savedData, null, 2)); vscode.window.showInformationMessage('Data exported'); }));
}

export function deactivate(): void { saveData(); disposeAll(); }